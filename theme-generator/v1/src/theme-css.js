...
function parseColor(value) {
  ...
  return {
    r: clampChannel(parseFloat(match[1])),
    g: clampChannel(parseFloat(match[2])),
    b: clampChannel(parseFloat(match[3]))
  };
}
...
function buildVariablesBlock(tokens) {
  ...
  return ['#_pi_surveyWidgetContainer {', ...lines, '}'].join('\n');
}
...
function validateTheme(tokens) {
  ...
  const widgetConfigs = tokens.widgets && typeof tokens.widgets === 'object' ? tokens.widgets : null;
  if (widgetConfigs) {
    Object.entries(widgetConfigs).forEach(([widgetName, config]) => {
      if (!config || typeof config !== 'object') return;
      const bg = parseColor(config.bg || config.background);
      const text = parseColor(config.text || config.foreground);
      if (!bg || !text) return;
      const ratio = contrastRatio(bg, text);
      if (ratio < 4.5) {
        const formatted = Number.isFinite(ratio) ? ratio.toFixed(2) : 'unknown';
        warnings.push(
          `widgets.${widgetName} contrast is ${formatted}:1 — background/text contrast may be insufficient.`
        );
      }
    });
  }
