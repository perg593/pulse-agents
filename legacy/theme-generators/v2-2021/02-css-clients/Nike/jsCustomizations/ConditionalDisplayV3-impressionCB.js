...
window.initConditionalDisplay = function(conditionalConfig){
  console.log(PulseInsightsObject.survey);
  ...
  Question.prototype.getSubUnits = function(){
    ...
    var i;
    for (i = 0; i < this.dependents.length; i+=1) {
      //get question el
  ...
  Question.prototype.attachEvents = function(){
    ...
    if(this.question.question_type == "single_choice_question"){
      ...
      if(this.question.button_type === 0 || this.question.button_type === 1){
        ...
        for (i = 0; i < els.length; i+=1) {
          ...
          elId = els[i].getAttribute("data-answer-id");
    ...
    // multiple choice
      ...
      var els = parentEl.querySelectorAll("li label");
      var keys;
      var i;
      for (i = 0; i < els.length; i+=1) {
  ...
  Question.prototype.getPulseQuestion = function(id){
    for (var i = 0; i < PulseInsightsObject.survey.questions.length; i+=1) {
  ...
  Question.prototype.getDependents = function(){
    ...
    for (var i = 0; i < keys.length; i+=1) {
      for (var j = 0; j < this.answers[keys[i]].length; j+=1) {
      ...
      if(typeof dependents !== "undefined"){
        for (var i = 0; i < dependents.length; i+=1) {
          ...
          if(typeof dependents[i].answers !== "undefined"){
            for (var j = 0; j < this.qListValues.length; j+=1) {
  ...
  Question.prototype.hideQuestions = function(checkCompleteness){
    ...
    for (var i = 0; i < keys.length; i+=1) {
      for (var j = 0; j < this.answers[keys[i]].length; j+=1) {
        ...
        if(this.subUnits[this.answers[keys[i]][j].id].button_type === 0 || this.subUnits[this.answers[keys[i]][j].id].button_type === 1){
          ...
          for (var k = 0; k < answerEls.length; k+=1) {
    ...
    if(this.qListValues.length > 0){
      var i;
      for (i = 0; i < this.qListValues.length; i+=1) {
  ...
  for (var i = 0; i < piInputs.length; i+=1) {
...
window.addEventListener("survey_loaded", function(){initConditionalDisplay(window.conditionalConfig)});

var surveyLoaded;
if(typeof(Event) === "function") {
  surveyLoaded =  new CustomEvent("survey_loaded");
}
else{
  surveyLoaded = document.createEvent("survey_loaded");
  surveyLoaded.initEvent("submit", true, true);
}

window.dispatchEvent(surveyLoaded);
