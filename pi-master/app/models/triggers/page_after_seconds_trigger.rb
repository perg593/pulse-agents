...
class PageAfterSecondsTrigger < Trigger
  validates :render_after_x_seconds_enabled, exclusion: [nil]
  validates :render_after_x_seconds, numericality: { greater_than: 0 }

  def summarize
    return unless render_after_x_seconds_enabled

    "#{render_after_x_seconds} seconds have elapsed"
  end
end
