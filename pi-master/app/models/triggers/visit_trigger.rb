...
class VisitTrigger < Trigger
  enum visitor_type: { all_visitors: 0, first_time_visitors: 1, repeat_visitors: 2 }

  validates :visits_count, numericality: { greater_than_or_equal_to: 0 }, if: -> { repeat_visitors? }

  def summarize
    if first_time_visitors?
      "First time visitors"
    elsif repeat_visitors?
      "Repeat visitors with at least #{visits_count} visits"
    elsif all_visitors?
      "All visitors"
    end
  end
...
#  url                                   :string(255)
#  url_matches                           :string
#  visitor_type                          :integer          default("all_visitors")
