...
class PageElementVisibleTrigger < Trigger
  validates_presence_of :render_after_element_visible
  validates :render_after_element_visible_enabled, exclusion: [nil]

  def summarize
    return unless render_after_element_visible_enabled

    "Element '#{render_after_element_visible}' must be visible"
  end
end
