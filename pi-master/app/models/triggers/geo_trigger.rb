...
class GeoTrigger < Trigger
  ...
  validates :geo_country, inclusion: { in: GeoTrigger.geoip_countries }

  validates :geo_state_or_dma, absence: true, if: -> { geo_country != "United States" }
  validates :geo_state_or_dma, inclusion: { in: (GeoTrigger.geoip_states + GeoTrigger.dma.map(&:last).flatten) },
    if: -> { geo_country == "United States" && geo_state_or_dma.present? }

  def summarize
    if geo_state_or_dma.present?
      "Pageview from #{geo_country}: #{geo_state_or_dma}"
    else
      "Pageview from #{geo_country}"
    end
  end
end
