...
class PreviousAnswerTrigger < Trigger
  validates :previous_possible_answer_id, :previous_answered_survey_id, presence: true

  def summarize
    "Answered survey #{Survey.find(previous_answered_survey_id).name} with possible answer #{PossibleAnswer.find(previous_possible_answer_id).content}"
  end
end
