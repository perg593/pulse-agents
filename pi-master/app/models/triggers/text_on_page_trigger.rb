...
class TextOnPageTrigger < Trigger
  validates :text_on_page_enabled, exclusion: [nil]
  validates :text_on_page_presence, exclusion: [nil]

  validates_presence_of :text_on_page_selector, :text_on_page_value

  def satisfied_with_page?(body)
    trigger_element = body.css(text_on_page_selector)

    text_found_on_page = trigger_element&.text&.include?(text_on_page_value)

    text_on_page_presence ? text_found_on_page : !text_found_on_page
  end

  def summarize
    "HTML element #{text_on_page_selector} must#{text_on_page_presence ? '' : ' not'} contain #{text_on_page_value}"
  end
end
