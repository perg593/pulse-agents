...
module Azurity
  class AzurityWorker
    ...
    def perform(
      ...
      report_data_start_date = start_date&.beginning_of_day || 1.week.ago.beginning_of_day
      report_data_end_date = end_date&.end_of_day || report_data_start_date.end_of_week
      ...
      Rollbar.error(e, "AzurityWorker failed")
    ...
    def azurity_account_info
      AZURITY_CONFIG[:azurity_accounts].detect { |account_info| account_info[:account_id] == @account.id }
    end

    def generate_account_csv_files(report_data_range, historical)
      csv_info_list = []

      Azurity.weekly_report_worker_account_ids.each do |account_id|
        @account = Account.find(account_id)

        tagged_logger.info "Processing Account(#{account_id})"
    ...
    def generate_account_csv(report_data_range, historical)
      ...
      brand_name = azurity_account_info[:brand_name]
    ...
    def upload_collated_file(csv_info_list, historical)
      collated_filename = if historical
        "PulseInsights_All_Brands_historical.csv"
      else
        "PulseInsights_All_Brands_#{Time.current.strftime("%d%m%y")}.csv"
      end

      collate_csv_files(csv_info_list, collated_filename)
      upload_copy_to_s3(collated_filename)
      upload_collated_file_to_azurity(collated_filename)
    end
    ...
    def upload_collated_file_to_azurity(collated_filename)
      return unless %w(production).include? Rails.env

      tagged_logger.info "Uploading collated file to Azurity via SFTP"

      sftp_credentials = Rails.application.credentials.azurity[:sftp]
