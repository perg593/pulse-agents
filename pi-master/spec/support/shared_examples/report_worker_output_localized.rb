...
RSpec.shared_examples "report worker output for localized survey" do
  describe "returns an accurate xlsx file" do
    ...
    it 'returns an accurate report overview sheet' do
      ...
      accurate_report_overview_sheet(reportee, worksheet)
    end

    it 'returns an accurate stats summary by day sheet' do
      worksheet = summary_by_day_worksheet(@xlsx_package)
      accurate_localized_summary_by_day_sheet(survey_locale_group, worksheet, date_format: "%m/%d/%y")
    end
